...
class Draw:
	...
	def __init__(self, level):
		...
		if self.current_level == 1:
			...
			self.cell_size = 30
		self.size = self._width, self._height = level.height * self.cell_size, level.width * self.cell_size
	...
	def draw_grid(self, pg):
		...
		for y in range(0, self._height, self.cell_size):
			pg.draw.line(self._display_surface, self.DARK_GRAY, (0, y), (self._width, y))

	def draw_lives(self, lives):
		pg.draw.rect(self._display_surface, self.BG_COLOR, (self._width - 150, 0, 150, 30))

		BASICFONT = pg.font.Font('freesansbold.ttf', 22)
		scoreSurf = BASICFONT.render('Lives: %s' % (lives), True, self.WHITE)
		scoreRect = scoreSurf.get_rect()
		scoreRect.topleft = (self._width - 120, 5)
		self._display_surface.blit(scoreSurf, scoreRect)

	def draw_score(self, score):
		pg.draw.rect(self._display_surface, self.BG_COLOR, (0, 0, 150, 30))
	...
	def draw_player(self, player):
		self.draw_player_items(*self.convert_to_pixel(*player.position), player.color)

	def draw_player_items(self, x, y, colour):
		self.draw_circle(x, y, colour, 2.5)
